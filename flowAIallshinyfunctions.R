...
library(flowCore)
library(plyr)
flow_auto_qc_last <- function(fcsfiles, remove_from = "all", output = 1, given_filename = FALSE,
  ...
  for (i in 1:length(set)) {
    if(given_filename != FALSE){
      filename_ext <- given_filename
    ...
    if (mini_report != FALSE) {
      minireport <-  paste0(folder_results, "/", filename, mini_report, ".txt")
